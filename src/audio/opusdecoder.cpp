...
QByteArray OpusDecoder::decodeK4Packet(const QByteArray &packet)
{
    ...
    if (encodeMode != 0x03) {
        ...
        if (!warned) {
            qWarning() << "OpusDecoder: Unexpected encode mode:" << encodeMode
                       << "(expected 0x03 for EM3 Opus Float)";
            warned = true;
        }
    ...
    quint16 frameSize = static_cast<unsigned char>(packet[4]) |
                        (static_cast<unsigned char>(packet[5]) << 8);
    ...
    for (int i = 0; i < monoSampleCount; i++) {
        ...
        if (monoFloats[i] > 1.0f) monoFloats[i] = 1.0f;
        if (monoFloats[i] < -1.0f) monoFloats[i] = -1.0f;
    }
...
{
    if (!m_decoder) return QByteArray();
    ...
    return QByteArray(reinterpret_cast<const char*>(pcm.constData()),
